...
public class MigrationServiceImpl extends DefaultComponent implements MigrationService {
    ...
    public MigrationStatus getStatus(String id) {
        ...
        if (step == null) {
            // migration never started, assumed at first state
            state = descr.states.keySet().iterator().next(); // LinkedHashMap guarantees order
            return new MigrationStatus(state);
        }
    ...
    public void runStep(String id, String step) {
        ...
        atomic(id, kv -> {
            ...
            MigrationStateDescriptor stateDescr = descr.states.get(state);
            if (stateDescr == null) {
        ...
        executor.submit(new MigratorWithContext(migrationContext -> {
            ...
            String state = migrationContext.isShutdownRequested() ? stepDescr.fromState : stepDescr.toState;
            atomic(id, kv -> {
                kv.put(id, state);
